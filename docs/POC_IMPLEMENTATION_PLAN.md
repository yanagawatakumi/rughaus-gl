...
- 管理画面タスク
  ...
  - 余力があれば商品単位の公開制御も検証
- テーマタスク
  - コレクション未公開時の導線（非表示 / 404回避）確認
  - ナビゲーションに非公開コレクションが露出しないことを確認
- 実装メモ
  - 公開判定はMarkets/Catalogを正本にし、テーマ側で独自判定を実装しない
  - 非公開対象のアクセス挙動はShopify標準に委譲する
- 主な確認対象ファイル
...
- T-03a: 非公開対象へのアクセスがShopify標準挙動で処理される
...
- C-02（中）: FR-03の直リンク時の表示整合
  - 理由: Markets/Catalog設定ミスが直接表示挙動へ影響する
  - 対策: 市場別公開設定とナビ露出の運用チェックをリリースゲート化
- C-03（中）: FR-07の翻訳品質保証
